...
class Game {
    ...
    randomColor() {
        this.nextColors = [Math.floor(Math.random() * 3) + 1, Math.floor(Math.random() * 3) + 1] 
    ...
    start() {
        ...
        setTimeout(() => {this.fallingPill(); this.draw()}, this.interval);
    ...
    spawnViruses(n) {
        ...
        for (var i = 0; i < n; i++) {
            ...
            if(positions.includes(x+"."+y)) { i--; continue; }
    ...
    canPillSpawn() { 
        return this.board.grid[0][3] == undefined && this.board.grid[0][4] == undefined;
    }
    ...
    fallingPill() {
        ...
        if(!this.userPill.canFall()) {
            ...
            setTimeout(() => {this.fallingPill()}, this.interval);
        }
    ...
    piecesClear() {
                ...
                for (let i = e.y[0]; i < endY+1; i++) {
                    for (let j = e.x[0]; j < endX+1; j++) {
                        ...
                        if(piece.constructor.name == 'Virus') {
                            ...
                            this.board.viruses.splice(this.board.viruses.indexOf(piece), 1);
                        } else {
                            const pill = piece.pill;
                            pill.pieces.splice(pill.pieces.indexOf(piece), 1);
                            if(pill.pieces.length == 0) {
                                this.board.pills.splice(this.board.pills.indexOf(pill), 1)
                            }
                            pill.spin = 0;
        ...
        if(cleared && this.anyVirusesLeft()) {setTimeout(() => { this.piecesFall(); }, this.interval);}
        else setTimeout(() => { 
